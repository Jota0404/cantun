...
export async function createService(input: Omit<Service, 'id' | 'createdAt' | 'updatedAt'>, id = crypto.randomUUID()): Promise<Service> {
...
export async function addServiceItem(input: Omit<ServiceItem, 'id' | 'updatedAt'>, id = crypto.randomUUID()): Promise<ServiceItem> {
  const item: ServiceItem = { ...input, id, updatedAt: new Date().toISOString() }
  await serviceItemRepository.create(item)
  return item
}

export async function createAssignment(input: Omit<Assignment, 'id' | 'createdAt' | 'updatedAt'>, id = crypto.randomUUID()): Promise<Assignment> {
  const now = new Date().toISOString()
