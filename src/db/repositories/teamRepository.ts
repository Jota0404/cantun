...
export class TeamRepository {
  ...
  async create(value: Team) { await this.db.teams.add(value) }
  ...
  async update(value: Team) { await this.db.teams.put(value) }
  async remove(id: string) { await this.db.teams.delete(id) }
}

export class TeamMembershipRepository {
  constructor(private readonly db: SalmodiaDatabase = defaultDb) {}

  async create(value: TeamMembership) { await this.db.teamMemberships.add(value) }
  ...
  async update(value: TeamMembership) { await this.db.teamMemberships.put(value) }
  async remove(id: string) { await this.db.teamMemberships.delete(id) }
}
