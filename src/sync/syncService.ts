...
import { TargetSyncEngine, type TargetEntity, type TargetEntityName } from './targetSyncEngine'
...
export async function queueTargetUpsert(entity: TargetEntityName, payload: TargetEntity) {
  if (!targetSyncEngine) return
  await targetSyncEngine.queueUpsert(entity, payload)
}

export async function queueTargetDelete(entity: TargetEntityName, entityId: string, updatedAt?: string) {
  if (!targetSyncEngine) return
