...
const EVENT_TYPES: readonly BandStageEventType[] = [
  'stage.snapshot', 'stage.play', 'stage.pause', 'stage.next', 'stage.previous', 'stage.goto', 'stage.set-key',
  'stage.prepare-next', 'stage.clear-prepared', 'stage.annotation-updated', 'stage.session-ended', 'stage.md-changed',
]
...
export function bandStageChannelName(sessionId: string): string { return `band-stage:${sessionId}` }

function isStageEvent(value: unknown): value is BandStageEvent {
  if (!value || typeof value !== 'object') return false
  const event = value as Partial<BandStageEvent>
  return typeof event.type === 'string' && EVENT_TYPES.includes(event.type as BandStageEventType) &&
    typeof event.sessionId === 'string' && typeof event.revision === 'number' && Number.isInteger(event.revision) &&
    event.revision >= 0 && typeof event.actorUserId === 'string' && typeof event.eventId === 'string' &&
    typeof event.sentAt === 'string' && 'payload' in event
}

export function createBandStageEvent<T>(input: { type: BandStageEventType; sessionId: string; revision: number; actorUserId: string; payload: T }): BandStageEvent<T> {
...
export class BandStageReconciler {
  ...
  constructor(options: BandStageRealtimeOptions) { this.options = options }
  ...
  async reconcile(reason: SnapshotReason = 'reconnect'): Promise<BandStageSnapshot> {
    ...
    if (snapshot.session.id !== this.options.sessionId) throw new Error('Snapshot de palco pertence a outra sessão.')
    if (snapshot.state.sessionId !== this.options.sessionId) throw new Error('Estado de palco pertence a outra sessão.')
  ...
  async acceptEvent(event: BandStageEvent): Promise<'applied' | 'ignored' | 'reconciled'> {
    if (event.sessionId !== this.options.sessionId || this.seenEventIds.has(event.eventId)) return 'ignored'
    if (event.revision <= this.currentRevision) { this.seenEventIds.add(event.eventId); return 'ignored' }
    if (event.revision > this.currentRevision + 1 && this.currentRevision >= 0) {
      await this.reconcile('revision-gap')
      if (event.revision <= this.currentRevision) { this.seenEventIds.add(event.eventId); return 'reconciled' }
    }
    ...
    if (event.type === 'stage.snapshot') {
      ...
      if (snapshot && snapshot.session.id === this.options.sessionId && snapshot.state.sessionId === this.options.sessionId && snapshot.state.revision >= this.currentRevision) {
        this.currentRevision = snapshot.state.revision
  ...
  private async loadSnapshot(): Promise<BandStageSnapshot> {
    const { data, error } = await this.options.client.rpc('get_band_stage_snapshot', { p_session_id: this.options.sessionId })
...
function isRowRecord(value: unknown): value is Record<string, unknown> { return Boolean(value && typeof value === 'object' && !Array.isArray(value)) }
function parseSnapshotPayload(payload: unknown): BandStageSnapshot | null {
  ...
  try { return { session: toBandStageSession(value.session), state: toBandStageState(value.state) } } catch { return null }
}
...
export class BandStageRealtime {
  ...
  constructor(options: BandStageRealtimeOptions) {
    ...
    this.channel = options.client.channel(bandStageChannelName(options.sessionId), { config: { private: true, broadcast: { self: false, ack: true } } })
    this.reconciler = new BandStageReconciler(options)
    this.channel.on('presence', { event: 'sync' }, () => { if (!this.disposed) this.emitPresence() })
    this.channel.on('presence', { event: 'join' }, () => { if (!this.disposed) this.emitPresence() })
    this.channel.on('presence', { event: 'leave' }, () => { if (!this.disposed) this.emitPresence() })
  ...
  get status(): BandStageConnectionStatus { return this.connectionStatus }
  ...
  async connect(): Promise<BandStageSnapshot> {
    ...
    this.connecting = this.connectInternal().catch((error) => { this.setConnectionStatus('ERROR'); throw error }).finally(() => { this.connecting = null })
  ...
  private async connectInternal(): Promise<BandStageSnapshot> {
    if (!this.subscribed) {
      await new Promise<void>((resolve, reject) => {
        let settled = false
        const settleStatus = (status: string, error?: Error) => {
          this.options.onStatus?.(status)
          if (settled) return
          if (status === 'SUBSCRIBED') { settled = true; this.subscribed = true; resolve(); return }
          if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') { settled = true; this.subscribed = false; reject(error ?? new Error(`Falha ao assinar sessão de palco: ${status}`)) }
        }
        const result = this.channel.subscribe(settleStatus)
        void Promise.resolve(result).then((status) => {
          if (typeof status === 'string') settleStatus(status)
        }).catch((error) => { if (!settled) { settled = true; this.subscribed = false; reject(error instanceof Error ? error : new Error('Falha ao assinar sessão de palco.')) } })
      })
    }
  ...
  async reconnect(): Promise<BandStageSnapshot> {
    ...
    this.reconnecting = this.reconnectInternal().catch((error) => { this.setConnectionStatus('ERROR'); throw error }).finally(() => { this.reconnecting = null })
  ...
  async trackPresence(payload: BandStagePresencePayload): Promise<void> {
    ...
    const normalizedPayload: BandStagePresencePayload = { ...payload, isMd: payload.userId === this.lastSnapshotMdUserId }
  ...
  async publish(event: BandStageEvent): Promise<void> {
    if (this.disposed) throw new Error('Sessão de palco já foi encerrada.')
    if (!this.subscribed) throw new Error('Canal de palco não está conectado.')
    try { await publishBandStageEvent(this.channel, event) } catch (error) { this.setConnectionStatus('ERROR'); throw error }
  }
