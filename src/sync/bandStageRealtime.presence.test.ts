...
describe('BandStageRealtime presence',()=>{
  it('tracks after the snapshot',async()=>{
    ...
    await realtime.trackPresence({userId:'md1',displayName:'João',musicalRole:'vocals',isMd:true,readiness:'waiting'})
    expect(client.channelRef.track).toHaveBeenCalledWith(expect.objectContaining({userId:'md1'}))
    expect(onPresence).toHaveBeenLastCalledWith([{userId:'md1',displayName:'João',musicalRole:'vocals',isMd:true,readiness:'waiting'}])
  })
  it('supports multiple participants and derives MD from snapshot',async()=>{
    const client=makeClient(), onPresence=vi.fn(), realtime=new BandStageRealtime({client:client as unknown as SupabaseClient,sessionId:'s1',onPresence})
    await realtime.connect()
    client.channelRef.emitPresence({a:[{userId:'md1',displayName:'João',musicalRole:'vocals',isMd:false}],b:[{userId:'bass1',displayName:'Lucas',musicalRole:'bass',isMd:true,readiness:'waiting'}]})
    expect(onPresence).toHaveBeenLastCalledWith([{userId:'md1',displayName:'João',musicalRole:'vocals',isMd:true,readiness:'waiting'},{userId:'bass1',displayName:'Lucas',musicalRole:'bass',isMd:false,readiness:'waiting'}])
  })
  ...
  it('retracks after reconnect and ignores presence after teardown',async()=>{
    const client=makeClient(), onPresence=vi.fn(), realtime=new BandStageRealtime({client:client as unknown as SupabaseClient,sessionId:'s1',onPresence})
    await realtime.connect()
    await realtime.trackPresence({userId:'u1',displayName:'Pedro',musicalRole:'electric-guitar',isMd:false,readiness:'waiting'})
    await realtime.reconnect()
    expect(client.channelRef.track).toHaveBeenCalledTimes(2)
    onPresence.mockClear()
    await realtime.disconnect()
    client.channelRef.emitPresence({c:[{userId:'late',displayName:'Late',musicalRole:'bass',readiness:'waiting'}]})
