...
const syncOrder: BandEntityName[] = ['bands','bandMembers','bandSongs','bandSetlistSongs','bandSongMemberStates','bandSetlists']
...
export class BandSyncEngine {
  ...
  constructor(db: SalmodiaDatabase, client: SupabaseClient) { this.db = db; this.client = client }
  ...
  async sync(userId: string) {
    ...
    this.syncing = true
    try { if (navigator.onLine) { await this.pushPending(userId); await this.pull(userId) } } finally { this.syncing = false }
    if (await this.db.bandSyncQueue.where('userId').equals(userId).count()) this.scheduleRetry(userId); else this.clearRetry()
  }
  private scheduleRetry(userId: string) { if (this.retryTimer !== undefined) return; this.retryTimer = window.setTimeout(() => { this.retryTimer = undefined; void this.sync(userId) }, RETRY_MS) }
