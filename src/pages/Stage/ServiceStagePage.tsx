...
export function ServiceStagePage() {
  const { stageSessionId: sessionId = '' } = useParams<{ stageSessionId: string }>()
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const loadedSongs = await getServiceStageSongs(sessionId)
        if (cancelled) return
  ...
  if (musicianView) {
    return (
      <main className="band-stage-page band-stage-page--musician" data-musical-role={musicalRole} style={{ '--band-stage-font-size': `${fontSize}px` } as React.CSSProperties}>
        ...
        <div className="band-stage-layout"><aside className="band-stage-setlist" aria-label="Ordem do serviço"><div className="band-stage-setlist__header"><strong>Ordem</strong><span>{songs.length}</span></div>{songs.map((song, index) => <div key={song.songId} className={index === activeIndex ? 'is-active' : ''} aria-current={index === activeIndex ? 'true' : undefined}><span>{index + 1}</span><strong>{song.title}</strong></div>)}</aside><section className="band-stage-content" aria-label={`Letra de ${activeSong.title}`}><div className="band-stage-content__toolbar"><div><button type="button" aria-pressed={readMode === 'scroll'} onClick={() => setReadMode('scroll')}>Rolagem</button><button type="button" aria-pressed={readMode === 'pages'} onClick={() => setReadMode('pages')}>Páginas</button></div><label>Tamanho <input aria-label="Tamanho da fonte" type="range" min="16" max="36" value={fontSize} onChange={(event) => setFontSize(Number(event.target.value))} /></label></div><article className={`band-stage-lyrics band-stage-lyrics--${readMode}`}>{displayedLyrics.split('\n').map((line, index) => <div key={`${index}-${line}`}>{line || '\u00a0'}</div>)}{experience.showNotes && activeSong.notes && <aside><strong>Observações</strong><p>{activeSong.notes}</p></aside>}</article></section></div>
