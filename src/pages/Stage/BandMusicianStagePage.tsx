...
export function BandMusicianStagePage() {
  ...
  if (loading) {
    return <main className="band-musician-stage"><div className="band-musician-stage__empty">Carregando palco…</div></main>
  }

  if (error && !snapshot) {
    return <main className="band-musician-stage"><div className="band-musician-stage__empty"><p role="alert">{error}</p><button type="button" onClick={() => navigate(-1)}>Voltar</button></div></main>
  }

  if (!snapshot || !activeSong) {
    return <main className="band-musician-stage"><div className="band-musician-stage__empty"><p>Nenhuma música disponível nesta sessão.</p><button type="button" onClick={() => navigate(-1)}>Voltar</button></div></main>
  }
  ...
  return (
    <main className="band-musician-stage" style={{ '--musician-font-size': `${fontSize}px` } as React.CSSProperties}>
      <header className="band-musician-stage__header">
        ...
        <div className="band-musician-stage__session">
          <span className={`band-musician-stage__status band-musician-stage__status--${snapshot.session.status}`}>
            {snapshot.session.status === 'live' ? 'Ao vivo' : snapshot.session.status === 'lobby' ? 'Lobby' : 'Encerrada'}
          </span>
      ...
      <section className="band-musician-stage__presence" aria-label="Estado da sessão">
        ...
        {snapshot.state.isRunning ? <span>Fluxo ativo</span> : <span>Fluxo pausado</span>}
      </section>

      {error && <p className="band-musician-stage__error" role="alert">{error}</p>}

      <div className="band-musician-stage__layout">
        <aside className="band-musician-stage__setlist" aria-label="Setlist da sessão">
          <div className="band-musician-stage__setlist-header"><strong>Setlist</strong><span>{songs.length}</span></div>
        ...
        <section className="band-musician-stage__content" aria-label={`Letra de ${activeSong.title}`}>
          <div className="band-musician-stage__toolbar">
            <div>
      ...
      <footer className="band-musician-stage__footer">
        ...
        <span>Os comandos de palco são controlados pelo MD. Este dispositivo apenas acompanha a sessão.</span>
      </footer>
    </main>
  )
}
