...
export function BandListPage() {
  ...
  // This effect synchronizes React state with the asynchronous local/remote band data source.
...
export function BandDetailPage() {
  ...
  // This effect synchronizes React state with the asynchronous band/member/invite data sources.
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => { void load() }, [load])
...
export function BandInvitePage() {
  const [params] = useSearchParams(); const { user } = useAuth(); const navigate = useNavigate(); const token = params.get('token') ?? ''; const [invite, setInvite] = useState<Awaited<ReturnType<typeof getBandInvite>>>(null); const [error, setError] = useState(token ? '' : 'Convite inválido.'); const [busy, setBusy] = useState(false)
