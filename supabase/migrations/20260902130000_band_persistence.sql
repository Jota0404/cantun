...
create unique index band_one_owner_idx
...
create policy "Band members can read bands"
  ...
  using (private.is_band_member(id));

create policy "Authenticated users can create owned bands"
...
create policy "Owners can delete membership"
...
create policy "Band members can read shared songs"
  on public.band_songs for select to authenticated
  using (private.is_band_member(band_id));

create policy "Band editors can insert shared songs"
  on public.band_songs for insert to authenticated
  with check (private.has_band_role(band_id, array['owner','editor']));

create policy "Band editors can update shared songs"
  on public.band_songs for update to authenticated
  using (private.has_band_role(band_id, array['owner','editor']))
  with check (private.has_band_role(band_id, array['owner','editor']));

create policy "Band editors can delete shared songs"
  on public.band_songs for delete to authenticated
  using (private.has_band_role(band_id, array['owner','editor']));

create policy "Members can read their song states"
  on public.band_song_member_states for select to authenticated
  using (user_id = (select auth.uid()) and private.is_band_member((select bs.band_id from public.band_songs bs where bs.id = band_song_id)));

create policy "Members can create their song states"
  on public.band_song_member_states for insert to authenticated
  with check (user_id = (select auth.uid()) and private.is_band_member((select bs.band_id from public.band_songs bs where bs.id = band_song_id)));

create policy "Members can update their song states"
  on public.band_song_member_states for update to authenticated
  using (user_id = (select auth.uid()))
  with check (user_id = (select auth.uid()));

create policy "Members can delete their song states"
  on public.band_song_member_states for delete to authenticated
  using (user_id = (select auth.uid()));

create policy "Band members can read band setlists"
  on public.band_setlists for select to authenticated
  using (private.is_band_member(band_id));

create policy "Band editors can insert band setlists"
  on public.band_setlists for insert to authenticated
  with check (private.has_band_role(band_id, array['owner','editor']) and created_by_user_id = (select auth.uid()));

create policy "Band editors can update band setlists"
  on public.band_setlists for update to authenticated
  using (private.has_band_role(band_id, array['owner','editor']))
  with check (private.has_band_role(band_id, array['owner','editor']));

create policy "Band editors can delete band setlists"
  on public.band_setlists for delete to authenticated
  using (private.has_band_role(band_id, array['owner','editor']));

create policy "Band members can read band setlist songs"
  on public.band_setlist_songs for select to authenticated
  using (exists (
    select 1 from public.band_setlists bs
    where bs.id = band_setlist_id and private.is_band_member(bs.band_id)
  ));

create policy "Band editors can insert band setlist songs"
  on public.band_setlist_songs for insert to authenticated
  with check (exists (
    select 1 from public.band_setlists bs
    where bs.id = band_setlist_id and private.has_band_role(bs.band_id, array['owner','editor'])
  ) and exists (
    select 1 from public.band_songs bso
    join public.band_setlists bs on bs.band_id = bso.band_id
    where bso.id = band_song_id and bs.id = band_setlist_id
  ));

create policy "Band editors can update band setlist songs"
  on public.band_setlist_songs for update to authenticated
  using (exists (
    select 1 from public.band_setlists bs
    where bs.id = band_setlist_id and private.has_band_role(bs.band_id, array['owner','editor'])
  ))
...
create policy "Band editors can delete band setlist songs"
  on public.band_setlist_songs for delete to authenticated
  using (exists (
    select 1 from public.band_setlists bs
    where bs.id = band_setlist_id and private.has_band_role(bs.band_id, array['owner','editor'])
  ));
